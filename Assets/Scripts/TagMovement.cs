...
public class TagMovement : MonoBehaviour
{
    ...
    {
        InitText();
        string selectString = ($"SELECT* FROM testpositions WHERE testnr = " +  _testNr);
        print(selectString);
        await GetData(selectString);
    }
                    ...
                    while (await reader.ReadAsync())
                    {
                        ...
                        if (_correct)
                        {
                            var o = new TagObject
                            {
                                ...
                                X = reader.GetDecimal(2) / 100 + 6,
                                Y = reader.GetDecimal(3) / 100
                            };
                            _tagObjects.Add(o);
                        }
                    }
